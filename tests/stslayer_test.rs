...
fn test_config_example() {
    ...
    let mut child = cmd
        ...
        .spawn()
        .unwrap();
    child.wait_timeout(Duration::from_millis(2500)).unwrap();
    child.kill().unwrap();
    child.wait().unwrap();
    ...
    for line in stdout_lines.by_ref().take(2) {
        assert!(
            Regex::new(
                r#"^\[\{"full_text":"Linux","min_width":"Linux","name":"command","instance":"kernel\ release","urgent":false,"separator":true,"markup":"none"\},\{"full_text":"\d{4}\-\d{2}\-\d{2}\ \d{1,2}:\d{2}:\d{2}","min_width":"\d{4}\-\d{2}\-\d{2}\ \d{1,2}:\d{2}:\d{2}","name":"command","instance":"date\ and\ time","urgent":false,"separator":true,"markup":"none"\}\],$"#
